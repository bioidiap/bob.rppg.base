...
base_expe_dir = 'experiments/paper/ssr-hci-protocols/'
...
extract = ['./bin/ssr_pulse.py', 'hci', '--dbdir', 'hci', '--bboxdir', 'bounding-boxes', '--outdir', str(pulse_dir), '--threshold', str(skin_threshold), '--stride', str(stride), '--skininit']
...
os.system('./bin/ssr_pulse.py hci --dbdir hci --bboxdir bounding-boxes --outdir ' + str(pulse_dir) + ' --threshold ' + str(skin_threshold) + ' --stride ' + str(stride) + ' -v --skininit')
