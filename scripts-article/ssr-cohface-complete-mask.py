...
base_expe_dir = 'experiments/paper/ssr-cohface-complete-mask/'
...
extract = ['./bin/ssr_pulse_from_mask.py', 'cohface', '--dbdir', 'cohface', '--bboxdir', 'bounding-boxes', '--pulsedir', str(pulse_dir), '--npoints', str(npoints), '--indent', str(indent), '--stride', str(stride)]
...
os.system('./bin/ssr_pulse_from_mask.py cohface --dbdir cohface --bboxdir bounding-boxes --pulsedir ' + str(pulse_dir) + ' --npoints ' + str(npoints) + ' --indent ' + str(indent) + ' --stride ' + str(stride) + ' -v')
