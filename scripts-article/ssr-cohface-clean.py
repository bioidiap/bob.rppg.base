...
base_expe_dir = 'experiments/paper/ssr-cohface-clean/'
...
extract = ['./bin/ssr_pulse.py', 'cohface', '--protocol', 'clean', '--dbdir', 'cohface', '--bboxdir', 'bounding-boxes', '--outdir', str(pulse_dir), '--threshold', str(skin_threshold), '--stride', str(stride)]
...
os.system('./bin/ssr_pulse.py cohface --protocol clean --dbdir cohface --bboxdir bounding-boxes --outdir ' + str(pulse_dir) + ' --threshold ' + str(skin_threshold) + ' --stride ' + str(stride) + ' -v')
