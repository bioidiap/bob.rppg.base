...
base_expe_dir = 'experiments/paper/ssr-xdb-cohface-hci/'
...
extract = ['./bin/ssr_pulse.py', 'hci', '--subset', 'test', '--dbdir', 'hci', '--bboxdir', 'bounding-boxes', '--outdir', str(pulse_dir), '--threshold', str(skin_threshold), '--stride', str(stride)]
...
os.system('./bin/ssr_pulse.py hci --subset test --dbdir hci --bboxdir bounding-boxes --outdir ' + str(pulse_dir) + ' --threshold ' + str(skin_threshold) + ' --stride ' + str(stride) + ' -v')
