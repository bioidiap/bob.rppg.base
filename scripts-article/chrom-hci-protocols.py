...
base_expe_dir = 'experiments/paper/chrom-hci-protocols/'
...
extract = ['./bin/chrom_pulse.py', 'hci', '--dbdir', 'hci', '--bboxdir', 'bounding-boxes', '--outdir', str(pulse_dir), '--threshold', str(skin_threshold), '--framerate', str(framerate), '--window', str(window), '--skininit']
...
os.system('./bin/chrom_pulse.py hci --dbdir hci --bboxdir bounding-boxes --outdir ' + str(pulse_dir) + ' --threshold ' + str(skin_threshold) + ' --framerate ' + str(framerate) + ' --window ' + str(window) + ' --skininit -v')

# computing heart-rate
os.system('./bin/rppg_get_heart_rate.py hci --indir ' + pulse_dir + ' --outdir ' + hr_dir + ' --framerate ' + str(framerate) + ' --nsegments ' +str(n_segments) + ' --nfft ' + str(nfft) + ' -v')
