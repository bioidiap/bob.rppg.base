...
base_expe_dir = 'experiments/paper/chrom-cohface-complete-mask/'
...
extract = ['./bin/chrom_pulse_from_mask.py', 'cohface', '--dbdir', 'cohface', '--bboxdir', 'bounding-boxes', '--pulsedir', str(pulse_dir), '--framerate', str(framerate), '--npoints', str(npoints), '--indent', str(indent), '--order', str(order), '--window', str(window)]
...
os.system('./bin/chrom_pulse_from_mask.py cohface --dbdir cohface --bboxdir bounding-boxes --pulsedir ' + str(pulse_dir) + ' --framerate ' + str(framerate) + ' --npoints ' + str(npoints) + ' --indent ' + str(indent) + ' --order ' + str(order) + ' --window ' + str(window) + ' -v')
