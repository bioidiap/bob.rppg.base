...
base_expe_dir = 'experiments/paper/ssr-hci-cvpr14/'
...
extract = ['./bin/ssr_pulse.py', 'hci', '--protocol', 'cvpr14', '--dbdir', 'hci', '--bboxdir', 'bounding-boxes', '--outdir', str(pulse_dir), '--threshold', str(skin_threshold), '--stride', str(stride), '--start', '306', '--end', '2136']
...
os.system('./bin/ssr_pulse.py hci --protocol cvpr14 --dbdir hci --bboxdir bounding-boxes --outdir ' + str(pulse_dir) + ' --threshold ' + str(skin_threshold) + ' --stride ' + str(stride) + ' --start 306 --end 2136 -v --skininit')

# computing heart-rate
os.system('./bin/rppg_get_heart_rate.py hci --protocol cvpr14 --indir ' + pulse_dir + ' --outdir ' + hr_dir + ' --framerate ' + str(framerate) + ' --nsegments ' +str(n_segments) + ' --nfft ' + str(nfft) + ' -v')
