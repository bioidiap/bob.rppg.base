...
base_expe_dir = 'experiments/paper/ssr-xdb-hci-cohface/'
...
#extract = ['./bin/ssr_pulse.py', 'cohface', '--protocol', 'clean', '--subset', 'test', '--dbdir', 'cohface', '--bboxdir', 'bounding-boxes', '--outdir', str(pulse_dir), '--threshold', str(skin_threshold), '--stride', str(stride), '--skininit']
...
os.system('./bin/ssr_pulse.py cohface --protocol clean --subset test --dbdir cohface --bboxdir bounding-boxes --outdir ' + str(pulse_dir) + ' --threshold ' + str(skin_threshold) + ' --stride ' + str(stride) + ' -v --skininit')
